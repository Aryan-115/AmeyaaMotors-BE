...
const createProduct = async (req, res) => {
    ...
    try {
        const { name, price, description, stock, colors, category,topspeed,range,battery,chargingtime} = req.body;
...
const getProducts = async (req,res) => {
    try {
        ...
        const products = await Product.find(query).select (
            "name price images rating description blacklist topspeed range battery chargingtime"
...
const removeFromBlacklist = async ( req,res)=>{
    ...
    try {
        ...
        return res.status(200).json({ success: true, message:`the product ${product.name} has been removed from blacklisted`,data: product})
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message });
    }
...
module.exports = {createProduct, updateProduct, deleteProduct,getProducts,getProductsByName,blacklistProduct,removeFromBlacklist};